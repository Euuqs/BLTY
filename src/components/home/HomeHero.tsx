...
export function HomeHero({ styleCount, scheduleCount, feedCount, categories, categoryTotal }: HomeHeroProps) {
  return (
    <motion.section
      ...
      initial="hidden"
      animate="show"
      className="grid grid-cols-12 gap-4 auto-rows-[minmax(160px,auto)]"
    >
      {/* ===== 主 Hero ===== */}
      <motion.div
        variants={item}
        className="col-span-12 lg:col-span-8 min-h-[440px] bento-tile flex flex-col justify-between relative overflow-hidden"
      >
        <Image
        ...
        {/* 水印字样 */}
        <span
          className="absolute bottom-20 -right-4 md:right-6 font-serif text-[5rem] md:text-[8rem] leading-none text-white/[0.05] tracking-tight select-none z-[1] pointer-events-none"
          aria-hidden="true"
        >
          柏里挑怡
        </span>
          ...
          <BentoTile className="flex-1 flex flex-col justify-center">
            ...
            <div className="flex items-center gap-3 mt-4 pt-4 border-t border-border/60">
              ...
              <div className="h-px flex-1 bg-gradient-to-r from-bai/40 via-cp/40 to-zhu/40 relative">
                <span className="text-cp text-[10px] absolute left-1/2 -top-2 -translate-x-1/2">
                  {"\u2665"}
                </span>
              </div>
        ...
        <motion.div variants={item} className="flex-1 flex">
          <BentoTile className="flex-1">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                <p className="font-serif text-2xl font-semibold text-bai">{styleCount}</p>
                <p className="text-[10px] text-muted font-mono mt-1">同款</p>
              </div>
              <div>
                <p className="font-serif text-2xl font-semibold text-zhu">{scheduleCount}</p>
                <p className="text-[10px] text-muted font-mono mt-1">行程</p>
              </div>
              <div>
                <p className="font-serif text-2xl font-semibold text-cp">{feedCount}</p>
