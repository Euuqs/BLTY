...
### 2026-09-01 · 复制历史内容快速填写

- 同款、行程和动态列表的每条内容新增“复制填写”入口，使用现有 Base64 URL 内容 ID 打开对应新建页面。
- 复制页服务端重新读取来源内容，不信任客户端提交的历史字段；来源 ID 无效或内容已删除时显示警告并退回空白新建表单。
- 新副本使用全新内容 UUID 和操作 UUID，状态始终重置为 `draft`，不会覆盖或修改来源 MDX；打开复制页本身不写仓库，保存后才执行构建、commit 和 push。
- 标题、日期、人物、类型或分类、正文、标签及各内容专属字段会完整预填，并给出需要重点复核的日期、时间、地点、来源链接或图片提示。
- 同款复制会由服务端根据来源 ID 重新取得旧图片路径；不上传图片时可继续复用，上传新图片则以新内容 ID 写入，来源内容与原图片不受影响。
- 列表中的“复制填写”和“编辑”均保持 44px 触控高度，操作区支持换行；复制成功和来源失效分别提供可读状态提示。
- 新增复制预填测试，完整写回回归由 22 项增至 23 项并全部通过；TypeScript、ESLint 和 Next.js 生产构建均通过。
- 浏览器实测三类列表和复制页均正确工作；同款字段、正文和图片预览完整预填，行程与动态标题正确预填；375px 下无横向溢出且保存、发布按钮保持 44px。
- 如后续转为云端编辑器，可复用同一复制预填层，并将来源读取替换为 GitHub 存储适配器。
